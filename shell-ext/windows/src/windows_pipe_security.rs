//! Ports `yadorilink_daemon::windows_pipe_security::PipeSecurityAttributes`
//! verbatim into this crate. Duplicated rather than shared: this crate
//! deliberately does not depend on `yadorilink-daemon` (see `Cargo.toml`'s
//! `[[bin]]` doc comment -- `yadorilink-cfapi-host` is meant to stay a thin
...
//! be refused a connection outright, not merely trusted to behave.
