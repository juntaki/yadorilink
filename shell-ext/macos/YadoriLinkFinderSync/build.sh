...
#
# WHY NO XCODE PROJECT (rather than the usual
# `xcodebuild -project... -scheme... build`): this repo's
# only real macOS build machine has Xcode's
...
#
# Usage (on a real macOS machine with Xcode installed):
...
# --- 4. Assemble: embed the extension in the host app -----
