...
fn ancestor_closure<S: CompactionDagStore>(
    ...
    group: &FolderGroupId,
    frontier: &[ChangeHash],
) -> Result<HashSet<ChangeHash>, ReplicaEngineError> {
...
pub fn plan_prune<S: CompactionStore>(
    ...
    group: &FolderGroupId,
    enrolled: &[DeviceId],
) -> Result<PrunePlan, ReplicaEngineError> {
    if enrolled.is_empty() {
...
pub fn checkpoint_supersedes<S: CompactionStore>(
    ...
    group: &FolderGroupId,
    change: &ChangeHash,
) -> Result<CheckpointSupersession, ReplicaEngineError> {
...
) -> Result<Option<ReBootstrapPlan>, ReplicaEngineError> {
