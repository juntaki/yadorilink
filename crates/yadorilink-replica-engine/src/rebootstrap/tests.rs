...
impl CompactionDagStore for FakeStore {
    fn heads(&self, _group: &FolderGroupId) -> Result<Vec<ChangeHash>, ReplicaEngineError> {
        Ok(self.heads.clone())
    }
...
fn unrelated_current_head_cannot_be_signed_under_checkpoint_history_base() {
    ...
    .unwrap_err();
    assert!(matches!(error, ReplicaEngineError::CorruptState(_)));
}
...
fn invalid_snapshot_content_never_reaches_atomic_installer() {
    ...
    let result = verify_and_install_rebootstrap(
        ...
        b"bad snapshot",
        |_manifest, _bytes| Err(ReplicaEngineError::CorruptState("snapshot hash mismatch".into())),
    );
    ...
    verify_and_install_rebootstrap(
        ...
        |_manifest, bytes| {
            if bytes == b"snapshot bytes" {
                ...
                Err(ReplicaEngineError::CorruptState("unexpected snapshot".into()))
            }
        },
    )
    .unwrap();
