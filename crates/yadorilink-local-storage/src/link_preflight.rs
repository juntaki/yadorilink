...
fn scan_directory(root: &Path) -> ScanResult {
    ...
    loop {
        ...
        //
        // The sync-root lock file (`sync_root_lock`) gets the identical
        // treatment as the artefact case, not the broader one: unlike the
        // legacy marker it is an exact, unambiguous name with no
        // look-alike-user-content case to protect, so a pre-existing file
...
mod tests {
    ...
    /// A pre-existing file that happens to sit exactly at the sync-root
    /// lock's path must be named as a collision before the user links the
    /// folder, same as a genuine artefact: this device will place its OS
    /// lock at exactly this path the moment the link starts.
    #[test]
    fn a_preexisting_sync_root_lock_named_file_is_reported_as_a_collision() {
        let dir = tempdir();
        std::fs::write(
        ...
        assert_eq!(
            ...
            vec![yadorilink_root_authority::sync_root_lock::SYNC_ROOT_LOCK_FILE_NAME.to_string()]
        );
        assert!(report.is_risky());
    }
