...
pub trait BlockStore: Send + Sync {
    ...
    /// against the stored bytes (`sync-performance` PERF-4).
    ///
    ...
    /// using `get`, per `security-hardening` SEC-5 — `get`'s mandatory
    /// verify-on-read is what limits SEC-5 to a DoS vector instead of a
    ...
    ///
    /// `sync-performance` PERF-8: this is called from async call sites
