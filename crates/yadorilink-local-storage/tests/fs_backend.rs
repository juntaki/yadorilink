...
/// A batch presence query correctly reports a
...
/// The batched `present_blocks` must return the
...
/// A genuine batch check should do far fewer
...
/// Before/after-style timing comparison: the trait's
...
fn present_blocks_batched_is_not_slower_than_naive_per_hash_default() {
    ...
    // batching must not be slower than the thing it
...
/// `get_unchecked` must return the exact same
...
fn get_unchecked_does_not_detect_corruption_unlike_get() {
    ...
    // *before* `get`, since `get`'s checksum-mismatch path now
...
/// Concurrently `put`-ing the exact
...
/// A torn/corrupted block self-heals on
...
fn a_torn_block_self_heals_on_a_subsequent_correct_put() {
    ...
    // Simulate the torn-block scenario: truncate the stored
...
/// Before/after timing signal: `get_unchecked` skips the
