...
/// `changes`; a parent this replica has since compacted still contributes
...
///
/// This MUST stay pruned-aware and agree with
/// `retained_history_integrity::validate_present_parent_shape`, which
/// recomputes the same value right after a change built from this result is
/// signed. A pruned-blind version here can sign a change with a Lamport
/// clock the very next validation step then rejects, and -- because a
/// delayed capture resolves the same fully-pruned basis on every retry --
/// that failure is not transient but a write that can never be published.
pub fn max_parent_lamport(
    conn: &Connection,
    group_id: &str,
    parents: &[ChangeHash],
    ...
    for p in parents {
        if let Some(l) = lamport_of(conn, p)? {
            max = max.max(l);
        } else if let Some(l) = pruned_lamport(conn, group_id, p)? {
            max = max.max(l);
        }
    }
    Ok(max)
