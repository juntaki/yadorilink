...
/// authenticate, a pruned stub would carry a wrong header forever, past the
/// one moment (startup repair) it could still be checked against the full
/// body.
#[test]
fn schema_init_fails_closed_on_an_authenticated_header_that_disagrees_with_its_encoded_change() {
    ...
    let hash = change.compute_hash();
    let wrong_header = vec![0xEEu8; 64];
    ...
    let error = init_dag_schema(&conn).expect_err(
        "a stored authenticated_header that disagrees with the change's own encoded body must \
         fail closed",
    );
    assert!(matches!(error, SyncError::CorruptState(_)));
}
