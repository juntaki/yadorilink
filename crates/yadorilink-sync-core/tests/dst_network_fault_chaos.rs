...
impl FaultProfile {
    fn from_seed(seed: u64) -> Self {
        ...
        let steady_loss = rng.gen_range(5..=30) as f64 / 100.0;
        let min_ms = rng.gen_range(15..=80);
        let max_ms = min_ms + rng.gen_range(40..=180);
        let partition_start = Duration::from_millis(rng.gen_range(20..=80));
        let partition_duration = Duration::from_millis(rng.gen_range(900..=1800));
        Self {
            steady_loss,
...
async fn run_scenario(
    ...
    for round in 0..ops_per_run {
        let path = CANDIDATE_PATHS[rng.gen_range(0..CANDIDATE_PATHS.len())];
        let kind_roll = if round == 0 { 9 } else { rng.gen_range(0..10) };
