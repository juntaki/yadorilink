...
fn run_scenario(seed: u64) -> Result<(), String> {
    ...
    // storage).
    let num_blocks = rng.gen_range(1..=3u32);
    let mut blocks = Vec::with_capacity(num_blocks as usize);
    let mut expected_content = Vec::new();
    let mut offset = 0u64;
    for _ in 0..num_blocks {
        let size = rng.gen_range(4..40u32);
    ...
    let left_stale_temp = rng.gen_bool(0.5);
    if left_stale_temp {
        let tmp_name = format!("{PATH}.yadorilink-tmp.{}.{}", std::process::id(), rng.gen::<u32>());
