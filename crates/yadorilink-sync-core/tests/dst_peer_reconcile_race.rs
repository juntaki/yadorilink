...
//! causally-later `IndexUpdate` for the *same path* arrives. Without
//! `PendingLocalChangeFlush` wired (`reconcile_one_file`'s guard,
//! `peer_session.rs`), A's stale local record makes B's update look like
//! a plain fast-forward (`VvOrdering::Before`), so A materializes B's
//! content over A's own real, not-yet-indexed edit -- silent data loss.
//! With the guard wired (this scenario's `SimDevice`, mirroring
//! `yadorilink-daemon::link_manager`'s real `LinkFlushHandle`/`impl
//! PendingLocalChangeFlush for DaemonState`), A's edit is force-flushed
//! and indexed *before* the comparison, correctly making it
...
fn setup_device_a(root: PathBuf, sync_state: Arc<SyncState>, store: Arc<FsBlockStore>) -> DeviceA {
    let processor = Arc::new(LocalChangeProcessor::new(sync_state, store, "device-a".to_string()));
...
async fn run_scenario(seed: u64, guard_enabled: bool, b_edit: BEdit) -> Result<(), String> {
    ...
    // Let the initial direct-path handshake + full-index exchange settle:
    // B adopts A's baseline (a plain first-adoption fetch/materialize,
    // exercising the real block-fetch path, not manual index
    // fabrication). Polled, not a single fixed sleep+check -- the direct
    // UDP path's own liveness/handshake retry timing is itself
    // seed-dependent and isn't part of what this scenario is testing;
    // only the race that follows needs precise timing control.
    ...
    session_b.send_index_update(GROUP_ID, vec![b_record]).await.map_err(|e| e.to_string())?;

    // Let A's PeerSyncSession::run() receive and process B's update,
    // including (if guard_enabled) A's own force-flush round trip.
