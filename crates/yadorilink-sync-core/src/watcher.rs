...
mod tests {
    ...
    async fn new_subdirectory_then_write_inside_it_does_not_deadlock_the_watcher() {
        ...
        let event = recv_until(
            ...
            "timed out waiting for the write inside a brand-new subdirectory -- the watcher is \
             likely deadlocked (fix-watcher-directory-registration-race-and-deadlock regression)",
        )
        .await;
        assert_eq!(event.kind, FsChangeKind::CreatedOrModified);
    ...
    async fn write_into_a_brand_new_directory_is_not_silently_missed() {
        ...
        let found = recv_until(
            ...
            "the write into a brand-new directory was never detected -- likely lost to the \
             registration race (fix-watcher-directory-registration-race-and-deadlock regression)",
        )
        .await;
    ...
    fn register_non_ignored_directories_does_not_hang_on_a_symlink_cycle() {
        ...
        std::thread::spawn(move || {
            ...
            let mut watcher =
                notify::recommended_watcher(|_res: notify::Result<Event>| {}).unwrap();
            let result = register_non_ignored_directories(
                &mut watcher,
                &mut watched,
                &root,
                &root,
            ...
            let _ = tx.send(result.map(|_| watched.len()));
        });

        let result = rx
            .recv_timeout(Duration::from_secs(10))
            .expect("register_non_ignored_directories hung on a self-referential symlink cycle");
        result.unwrap();
    }
}
