...
pub enum MaterializationState {
    ...
    Hydrating,
    /// Eviction has started but the placeholder has not been committed yet.
    /// Watchers must continue treating disk changes in this state as possible
    /// user data rather than suppressing them as placeholder self-echoes.
    Evicting,
}
...
impl MaterializationState {
    ...
    pub fn from_db_str(s: &str) -> Self {
        match s {
            ...
            "evicting" => MaterializationState::Evicting,
...
/// [`crate::index::PendingEnrollment`] marker belongs to — mirrors the
/// coordination plane's CREATE vs JOIN prepare/activate/cancel routes.
...
impl EnrollmentKind {
    ...
    pub fn from_db_str(s: &str) -> Self {
        match s {
            "join" => EnrollmentKind::Join,
            _ => EnrollmentKind::Create,
        }
    }
}

/// The kind of filesystem entry a file-index row represents — deliberately **not** a
...
/// materialization-side apply. Always `false` on non-Unix platforms (no
/// error, no attempted read of a bit that doesn't exist there).
///
...
/// no-op-and-always-`false` Windows/other-platform counterpart
/// explicitly required (Windows has no owner-exec permission bit to read).
#[cfg(not(unix))]
