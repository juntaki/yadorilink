...
pub fn reconstruct_file(
    ...
    // behind: nothing revisits it until the next startup's
    // `cleanup_stale_temp_files` sweep, and until then a whole-directory scan
    // would see a stray partial/empty file at this path. Remove it on every
    // error path so an interrupted reconstruct leaves the directory as it
    // found it (the on-disk target is untouched — the rename is the only step
    // that publishes anything under `out_path`).
    let assemble = || -> Result<(), SyncError> {
...
#[cfg(unix)]
fn sync_parent_directory(path: &Path) -> Result<(), SyncError> {
...
#[cfg(not(unix))]
fn sync_parent_directory(_path: &Path) -> Result<(), SyncError> {
    Ok(())
}
