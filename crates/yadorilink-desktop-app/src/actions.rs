...
use yadorilink_ipc_proto::daemonctl::{
    DiagnosticsExportRequest, LimitsSetRequest, LinkRequest, PauseRequest, ResumeRequest,
    ShutdownRequest, UpdateCheckRequest, UpdateInstallRequest,
};
...
/// so a folder chosen through this app's (OS-native) folder picker is
/// treated the same as an explicit `--yes` from the CLI. The daemon's own
/// `Link` handler still runs its independent preflight and can still
/// reject an unsafe link outright (e.g. nested inside an existing link) —
/// this app adds no bypass of that.
pub async fn link_folder(local_path: String, group_id: String) -> Result<(), IpcError> {
...
pub enum AddFolderError {
    #[error(transparent)]
    Ipc(#[from] IpcError),
    #[error("not signed in — run `yadorilink login` (or complete first-run setup) first: {0}")]
    NotSignedIn(String),
    #[error("could not resolve folder group {0:?}: {1}")]
...
/// that's reused rather than re-implemented here), then links exactly
/// like `link_folder` above. Returns a clear, distinguishable error when
/// no device is signed in yet, rather than a confusing gRPC failure —
/// `main.rs` only offers this action once
/// `ipc_client::is_device_registered()` is true, but the access token
/// itself (populated by a separate `yadorilink login`) can still be
/// missing/expired, hence this still needs its own explicit case.
pub async fn add_synced_folder(
    local_path: String,
    group_name: String,
) -> Result<(), AddFolderError> {
    let access_token = yadorilink_cli::grpc::require_access_token()
        .map_err(|e| AddFolderError::NotSignedIn(e.to_string()))?;
    let group_id = yadorilink_cli::grpc::resolve_group_id(&access_token, &group_name)
        .await
        .map_err(|e| AddFolderError::GroupResolution(group_name, e.to_string()))?;
    link_folder(local_path, group_id).await?;
    Ok(())
}
...
/// "not implemented on this platform yet" stance as `login_item.rs`'s
/// Windows stub) since a native Win32 folder picker needs either a COM
/// `IFileOpenDialog` call or an extra crate this pass didn't add.
#[cfg(target_os = "macos")]
pub fn pick_folder() -> Option<std::path::PathBuf> {
    ...
    if !output.status.success() {
        return None; // user cancelled, or osascript unavailable
    }
...
pub fn prompt_text(message: &str) -> Option<String> {
    let script = format!(
        "text returned of (display dialog {} default answer \"\" with title \"YadoriLink\")",
...
#[cfg(not(target_os = "macos"))]
pub fn prompt_text(_message: &str) -> Option<String> {
    None
}

/// Escapes `s` for embedding as an AppleScript string literal — the
/// message text here is always a fixed, this-crate-authored constant
/// (never raw user/network input), but quoting it properly is still
