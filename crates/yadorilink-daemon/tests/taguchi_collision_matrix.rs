...
//! duplicated from `e2e_three_devices.rs`/`monkey_chaos.rs`/
...
async fn run_taguchi_row(
    ...
    // matching monkey_chaos.rs's own reasoning: name-set convergence
    // (checked above, since it's cheap to poll) doesn't guarantee content
    // has finished propagating too.
