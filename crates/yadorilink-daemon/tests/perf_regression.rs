...
//! scenarios in this task already have dedicated before/after
//! measurements elsewhere: "frequent-modify re-hash avoidance" in
//! `yadorilink-sync-core/src/local_change.rs`'s
//! `unchanged_size_and_mtime_skips_rechunking_entirely` (PERF-2), and
...
//! instead of a wall-clock speed comparison, each test proves PERF-1's
...
//! practice and would reliably FAIL under the pre-PERF-1 code (a
...
/// PERF-1 (`link_manager.rs`'s wrapping of the initial `scan_existing_files`
...
/// PERF-1 (`hydration.rs`'s `spawn_blocking` wrap around `BlockStore::put`
...
async fn large_file_hydration_does_not_block_concurrent_async_work() {
    ...
    let hub_source = RelayHub::connect(relay_addr, secret_source.clone()).await.unwrap();
    let hub_dest = RelayHub::connect(relay_addr, secret_dest.clone()).await.unwrap();
    ...
    // their own handshake with each other over the relay channel before
