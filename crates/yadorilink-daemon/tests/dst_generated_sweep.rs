...
async fn connect_sessions(
    ...
    // since `ChangeAuthenticator`/`PendingLocalChangeFlush` are now
    // construction-only `PeerSyncSessionDeps` fields, not post-hoc setters.
    ...
    // under fault (the authenticator above is now already wired at
    // construction).
...
async fn converge_path(a: &ChaosDevice, b: &ChaosDevice, path: &str) -> bool {
    dst_support::settle::settle_until(PER_PATH_CONVERGE_BUDGET, || {
        let fa = a.state.file_index_repository().get_file(GROUP_ID, path).ok().flatten();
        let fb = b.state.file_index_repository().get_file(GROUP_ID, path).ok().flatten();
        match (&fa, &fb) {
            (None, None) => true,
            (Some(x), Some(y)) => {
                // Convergence is identity of the authoring change, not equality
                // of a per-file counter: two replicas agree exactly when the
                // same signed change authored the row they each project.
                x == y
                    && a.state
...
async fn drive_case(case: &Case, profile: &HarnessProfile) -> Result<Vec<Violation>, String> {
        ...
        for (d, sub) in subops {
            ...
            match &sub {
                Op::Write { path, content_id } | Op::Edit { path, content_id } => {
                    ...
                    .await;
                    oracle.record_write(path, d, *content_id, authoring_of(device, path));
                ...
                Op::Delete { path } => {
                    ...
                    oracle.record_delete(path, d, authoring_of(device, path));
                    reference_ops.push(RefOp {
                        path: path.clone(),
                        device_id: device.device_id.clone(),
                        round: ref_round,
    ...
    let devices_ref: Vec<(&Path, &ReplicaCoordinator)> = vec![
    ...
    violations.extend(oracle.check_no_loss(&content_table, &devices_ref, GROUP_ID));
...
enum RunOutcome {
    ...
    /// An infra condition unrelated to sync correctness (startup stall
    /// under host load, madsim time limit, OS thread-creation ceiling).
...
fn dst_generated_sweep() {
    ...
    // a run is explicitly targeted (DST_BASE_SEED), the reproduction recipe, or
    // via the dedicated `DST_SKIP_CORPUS_REPLAY` flag -- see
    // `dst_support::corpus::should_skip_replay`'s doc comment. Deliberately a
    // separate check from `targeted` below: that flag also bounds
    // `max_attempts` for the fresh sweep, a different concern this must not
    // perturb.
