...
async fn concurrent_create_of_a_windows_reserved_basename() {
    ...
    if cfg!(windows) {
        ...
        // Wait for the conflict-copy artifact to actually exist instead.
        wait_until_with_context(
            || {
                let a = real_entry_names(device_a.root.path());
                let b = real_entry_names(device_b.root.path());
...
async fn concurrent_create_with_trailing_dot_or_space() {
    ...
    if cfg!(windows) {
        ...
        // sync converges -- never a conflict of any kind.
        wait_until_with_context(
            || {
                let a = real_entry_names(device_a.root.path());
                let b = real_entry_names(device_b.root.path());
                a == b
                    && a.contains(&"notes.txt.".to_string())
                    && a.contains(&"notes.txt ".to_string())
            },
            Duration::from_secs(20),
            || {
...
async fn concurrent_create_with_illegal_windows_characters() {
    ...
    if cfg!(windows) {
        ...
        // particular test happens to run on a non-Windows one.
        wait_until_with_context(
            || {
                let a = real_entry_names(device_a.root.path());
                let b = real_entry_names(device_b.root.path());
                a == b && a.contains(&name_a.to_string()) && a.contains(&name_b.to_string())
            },
            Duration::from_secs(20),
            || {
                format!(
...
async fn long_path_near_windows_max_path_length() {
    ...
    wait_until_with_context(
        || {
            let a = real_entry_names(&dir_a);
            let b = real_entry_names(&dir_b);
            a == b && a.iter().any(|n| is_conflict_copy(n))
        },
        Duration::from_secs(20),
        || {
