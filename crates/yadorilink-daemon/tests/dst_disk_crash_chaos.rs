...
async fn connect_sessions(
    ...
    *device_b.session.lock().unwrap_or_else(|p| p.into_inner()) = Some(session_b.clone());
    ...
    // whether the content is actually reachable.
...
fn restart_recovery(
    ...
    device_id: &str,
) -> Result<Arc<ReplicaCoordinator>, String> {
    let state = Arc::new(ReplicaCoordinator::open(db_path).map_err(|e| e.to_string())?);
...
async fn run_scenario(seed: u64) -> Result<(Vec<String>, Vec<String>), String> {
    ...
    {
        let state = ReplicaCoordinator::open(&db_a).map_err(|e| e.to_string())?;
    ...
    {
        let state = ReplicaCoordinator::open(&db_b).map_err(|e| e.to_string())?;
    ...
    let devices: Vec<(&Path, &ReplicaCoordinator)> =
...
fn disk_fault_crash_restart_chaos_scenario() {
    ...
    // Every other DST sweep carries this zero-coverage guard; this file was
    // the one missing it. Without it, a harness regression that pushes
    // every seed into the time-limit skip (the same silent-zero-coverage
    // shape the missing convergence-driver wiring produced elsewhere)
    // reports green while exercising nothing.
    assert!(
