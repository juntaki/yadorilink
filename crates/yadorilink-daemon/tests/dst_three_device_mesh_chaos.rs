...
//! loop is `reconcile_one_file`'s `ChangeOrdering::Equal | After => no-op`
...
//! then C's forwarder forwards it to B over C-B) -- two arrival paths for
...
//! `GlobalOracle`'s checks already take `devices: &[(&Path, &ReplicaCoordinator)]`,
...
impl ChangeAuthenticator for PinnedAuthenticator {
    fn signing_key(&self, device_id: &str) -> Option<[u8; 32]> {
        self.verifying_keys.get(device_id).copied()
    }
...
/// A deterministic per-device Ed25519 signing key -- test-only, derived from
/// a fixed distinct seed byte per device so a device's key (and thus every
/// change hash it authors) is stable across runs of the same scenario seed
/// without perturbing the workload RNG stream.
fn device_signing_key(seed_byte: u8) -> SigningKey {
    SigningKey::from_bytes(&[seed_byte; 32])
}

/// How often each session re-announces its DAG heads to its peer. Production
/// runs a 90s periodic frontier audit (`DEFAULT_MAINTENANCE_RECONCILE_INTERVAL`);
/// this harness dials it to a fast cadence so a signed local commit reaches
/// every peer within a round's settle window over the real
/// `HeadsAnnounce -> ChangeRequest -> ChangeBatch` negotiation the `run` loop
/// carries, rather than waiting on the production interval. A test-harness
/// timing measure, not a production behavior change.
const DAG_AUDIT_INTERVAL: Duration = Duration::from_millis(150);
...
/// fewer.
///
/// 150s, not the earlier 75s: the same lost-`BlockRequest` recovery path
/// the two-device harness already relaxed its gate for (a transport-level
/// datagram loss recovers only via `DEFAULT_HYDRATION_TIMEOUT`, 30s, plus
/// bounded retry) can happen TWICE in sequence here — content reaching the
/// far device through the mesh's relay hop (A→B, then B→C) chains two
/// independent recoveries, so the worst legitimate settle path is roughly
/// double the two-device one, not merely "a bit above" it. Observed live
/// (~1-in-40 heavy-sweep seeds): the failure-path diagnosis dump showed
/// all three devices byte-identical (same heads, same record, same
/// on-disk content) immediately after the 75s gate expired — convergence
/// had happened, marginally too late. Simulated time, so a generous value
/// costs nothing on fast seeds; the oracle's convergence-promptness check
/// still flags (without failing) genuinely slow rounds.
const ROUND_PROGRESSION_GATE: Duration = Duration::from_secs(150);
// The *budget* the shared
// `settle` primitive is given for the terminal convergence wait, not a
// hand-rolled poll gate -- the 90s rationale still governs the value,
// widened to the same chained-relay-recovery bound as
// ROUND_PROGRESSION_GATE above (the terminal wait can hit the identical
// double-recovery worst case).
const FINAL_CONVERGENCE_BUDGET: Duration = Duration::from_secs(150);
...
impl MeshDevice {
    /// Announces this device's advanced DAG heads to every neighbor session,
    /// the change-history analogue of the daemon's `broadcast_change`
    /// fan-out. A locally-produced edit has already been emitted as a signed
    /// change into this device's store by its emitter-wired `process_event`/
    /// `process_flush`; this just re-announces heads so each peer discovers it
    /// immediately (the short-interval periodic frontier audit in each
    /// session's `run` loop is the backstop). Fanning out to *all* of this
    /// device's sessions, with no source-peer exclusion, is deliberate --
    /// exactly `broadcast_change`'s shape (see this file's doc comment).
    async fn broadcast(
        &self,
        group_id: &str,
...
impl MeshDevice {
    ...
    /// direct, disk-authoritative `process_event` on this exact path,
    /// before the incoming remote change is admitted. This harness used to
    /// `return` instead, making every scenario built on it exercise a flush
    /// guard strictly weaker than the production one, on precisely the
    /// branch a lost local write lands on (`dst_network_fault_chaos`'s
    /// `[NoLoss]` violations, seeds 3298840576/3298840578, were exactly
    /// this).
    ///
    /// Same `CreatedOrModified`-only, skip-if-absent discipline as
    /// production — see that method's doc comment for why synthesizing
    /// `Removed` here would corrupt the very comparison the caller is about
    /// to make.
    async fn capture_undiscovered_local_change(&self, group_id: &str, path: &Path) {
        ...
        self.broadcast(group_id, records).await;
    }
}
...
impl PendingLocalChangeFlush for MeshDevice {
    fn flush_pending_local_change<'a>(
        ...
        Box::pin(async move {
            ...
            // Mirrors production's `LinkFlushHandle::flush_pending_local_change`
            // -- see `capture_undiscovered_local_change` above for why a bare
...
async fn connect_edge(
    ...
    // fails closed ("no BlockServeEngine installed") regardless of whether
    // the content is actually reachable on the other side.
    ...
    session_b.set_block_serve_engine(yadorilink_peer_session::block_serve::BlockServeEngine::new(
        u64::MAX,
        u64::MAX,
        u64::MAX,
        1_000,
...
async fn connect_mesh(
    ...
    for (device, sessions) in [
        ...
        (device_c, [session_bc_c, session_ac_c]),
    ] {
        dst_support::convergence_driver::spawn_convergence_driver(
            device.state.clone(),
            sessions.iter().map(Arc::downgrade).collect(),
            vec![GROUP_ID.to_string()],
        );
    }
}
...
/// ancestry. `None` leaves the entry un-superseded, which fails loud.
/// Reads the change this device authored for `path` -- the causal evidence
/// the oracle's supersession test runs on.
///
/// **Settles through the real flush primitive first, and must.**
/// `dag_last_authored_change_for_path` reads the live index, so asking
/// before this device's own local write has flushed returns `None` -- and a
/// `None` evidence fails loud as "never causally superseded", reporting
/// data loss on a run that converged perfectly. `deliver_local_write` only
/// queues a watcher event; it does not author anything, so "record after
/// delivering" is not sufficient on its own.
///
/// Measured: this was the entire `[NoLoss]` population of these scenarios.
/// On `dst_two_device_chaos` seed 3298840576 the trace shows the product
/// authoring both racing sides as genuine concurrent siblings off the same
/// parent and producing a conflict copy (`copies=1`) -- nothing was ever
...
async fn deliver_local_delete(device: &Arc<MeshDevice>, path: &str) -> Result<(), String> {
    // `fs_ops::remove` tolerates a concurrent removal (the spawned session/
    // debounce tasks share this simulated runtime and can race on the same
    // file) exactly as the old local `remove_file_if_present` did.
...
/// common base before a race round, a legitimate `ChangeOrdering::Before`
...
async fn converge_path_all3(devices: [&MeshDevice; 3], path: &str) -> (bool, Duration) {
    ...
    // not a version vector, so a change projected onto a receiver lands with
    // an empty version vector (the authoring device keeps a richer one) and
    // cross-device VV equality can never hold on this wire. Comparing the
    // bytes actually on disk (all three identical, or all three absent) is the
    // faithful "genuinely converged common base" this gate needs before a race
    // round, and matches the byte-level snapshot the terminal
    // `check_convergence` oracle uses -- if anything a stricter base than the
    // metadata-only VV compare it replaces.
    let outcome = dst_support::settle::settle_until(ROUND_PROGRESSION_GATE, || {
        let contents: Vec<Option<Vec<u8>>> =
            devices.iter().map(|d| std::fs::read(d.root.join(path)).ok()).collect();
        match (&contents[0], &contents[1], &contents[2]) {
            (None, None, None) => true,
    ...
    if !outcome.converged {
        // Same DAG-level diagnosis monkey_chaos.rs grew for exactly this
        // shape of failure: without it, "never converged" cannot be told
        // apart between delivery (heads differ), projection (heads equal,
        // record missing/stale on one device), and materialization
        // (records equal, bytes differ).
...
async fn run_scenario(seed: u64, ops_per_run: usize) -> Result<(), String> {
    ...
    let key_b = device_signing_key(0x22);
    let key_c = device_signing_key(0x33);
    let authenticator = Arc::new(PinnedAuthenticator {
        verifying_keys: HashMap::from([
            ("device-a".to_string(), key_a.verifying_key().to_bytes()),
            ("device-b".to_string(), key_b.verifying_key().to_bytes()),
            ("device-c".to_string(), key_c.verifying_key().to_bytes()),
        ]),
    });
    ...
    // strictly-monotonic synthetic clock for this run, owned by
    // `dst_support::clock::HarnessClock`. `fs_ops::write` stamps every tempdir
    // mutation through it, and every advance keeps the session-visible
    // `now_unix_nanos` override in lockstep. Seeded from `seed` itself so
    // different seeds explore different tie-break regions -- the full rationale
    // now lives in `clock.rs`.
    ...
    for round in 0..ops_per_run {
        ...
        clock.tick_round();
        let path = CANDIDATE_PATHS[rng.random_range(0..CANDIDATE_PATHS.len())].to_string();
        ...
        match kind {
            RoundKind::Solo => {
                ...
                let device_idx = rng.random_range(0..3);
                let device = device_by_idx(device_idx);
                let do_delete = rng.random_bool(0.2) && device_has_live_record(device, &path);
                if debug {
                    eprintln!(
                ...
                if do_delete {
                    ...
                    oracle.record_delete(&path, device_idx, authoring_of(device, &path).await);
                    recorded_ops.push((
                        device_idx,
                        round as u64,
            ...
            RoundKind::Race => {
                ...
                // Recorded only once the write is actually applied: the oracle's
                // causal evidence is the authoring change x really wrote, which
                // does not exist before delivery.
                oracle.record_write(&path, x_idx, x_content_id, authoring_of(x, &path).await);
                tokio::time::sleep(RACE_INNER_DELAY).await;

                // Deliberate mid-race spacing: nudge the shared clock +100ms so
                // `y`'s stamped mtime lands strictly after `x`'s pending write
                // (DAG ancestry decides the conflict; this keeps the
                // synthetic timeline honest). Kept as an explicit `advance`, not
                // a `tick_round`.
                ...
                if y_deletes {
                    ...
                    oracle.record_delete(&path, y_idx, authoring_of(y, &path).await);
                    ...
                    oracle.record_write(&path, y_idx, y_content_id, authoring_of(y, &path).await);
                    recorded_ops.push((
                        y_idx,
                        round as u64,
            ...
            RoundKind::RenameRace => {
                ...
                let renamer_idx = rng.random_range(0..3);
                ...
                if !device_has_live_record(renamer, &path) {
                    ...
                    deliver_local_write(renamer, &path, content.clone(), &clock).await?;
                    tokio::time::sleep(ROUND_SETTLE).await;
                    let content_id = next_content_id;
                    next_content_id += 1;
                    content_table.insert(content_id, content);
                    oracle.record_write(
                        ...
                        renamer_idx,
                        content_id,
                        authoring_of(renamer, &path).await,
                    );
                    recorded_ops.push((
                        renamer_idx,
                        round as u64,
                ...
                let racer_idx = (renamer_idx + 1 + rng.random_range(0..2)) % 3;
                let racer = device_by_idx(racer_idx);
                let race_old = rng.random_bool(0.5);
                if debug {
                    eprintln!(
                ...
                tokio::time::sleep(RACE_INNER_DELAY).await;
                // Deliberate mid-race spacing (+100ms), kept as an explicit
                // `advance` so the racer's contesting mutation lands strictly
                // after the tombstone on the synthetic timeline.
                clock.advance(100_000_000);

                if race_old {
                    let racer_deletes =
                        rng.random_bool(0.3) && device_has_live_record(racer, &path);
                    if racer_deletes {
                        dst_support::fs_ops::remove(&racer.root.join(&path))?;
                        apply_and_push(racer, &path, FsChangeKind::Removed).await?;
                        oracle.record_delete(&path, racer_idx, authoring_of(racer, &path).await);
                        recorded_ops.push((
                            racer_idx,
                            round as u64,
                            Op::Delete { path: path.clone() },
                        ...
                        dst_support::fs_ops::write(&clock, &racer_path, &racer_content)?;
                    ...
                    oracle.record_delete(&path, renamer_idx, authoring_of(renamer, &path).await);
                    recorded_ops.push((
                        renamer_idx,
                        round as u64,
                        Op::Delete { path: path.clone() },
                ...
                if !race_old {
                    deliver_local_write(renamer, &new_path, new_content.clone(), &clock).await?;
                    tokio::time::sleep(RACE_INNER_DELAY).await;
                    // Deliberate mid-race spacing (+100ms), explicit `advance`.
                    clock.advance(100_000_000);
                    ...
                    oracle.record_write(
                        ...
                        renamer_idx,
                        new_content_id,
                        authoring_of(renamer, &new_path).await,
                    );
                    recorded_ops.push((
                        renamer_idx,
                        round as u64,
                    ...
                    oracle.record_write(
                        ...
                        racer_idx,
                        racer_content_id,
                        authoring_of(racer, &new_path).await,
                    );
                    recorded_ops.push((
                        racer_idx,
                        round as u64,
    ...
    // informational `RepairedBySweep` finding is surfaced,
...
fn run_in_madsim(seed: u64, ops_per_run: usize) -> Result<(), String> {
    ...
    // Generous margin above the WORST legitimate simulated-time spend: the
    // per-round and terminal settle budgets are 150s each now (see
    // ROUND_PROGRESSION_GATE — the chained-relay recovery rationale), so
    // the old 180s ceiling (sized against the previous 90s budget) could
    // trip DURING a legitimately slow settle and reclassify it as a
    // TIME_LIMIT skip, silently shrinking coverage. Simulated time, so a
    // generous ceiling costs nothing on healthy seeds; the ceiling exists
    // only to convert a genuinely hung simulation into a classifiable
    // skip rather than a wedged test process.
...
fn three_device_mesh_chaos_scenario() {
    ...
    // Skipped entirely for a targeted run -- see
    // `dst_support::corpus::should_skip_replay`'s doc comment.
