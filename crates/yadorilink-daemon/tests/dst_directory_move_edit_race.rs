...
//! `DEFAULT_MAINTENANCE_RECONCILE_INTERVAL`, well past the 60s per-seed time
...
async fn connect_sessions(
    ...
    // driver, since there is nothing on A's side to answer a `BlockRequest`.
...
async fn run_scenario(seed: u64, ordering: Ordering) -> Result<(), String> {
    ...
    // (`content_present_anywhere`), so it needs the same test-only driver as
    // the other migrated DST scenarios. Deliberately not routed through
    // `wire_dag_session` (see the comment at its call site above) -- only the
    // materialization piece is needed here, not the cadence/authenticator
    // wiring, which this scenario controls by hand.
    dst_dag_migrate_b2::spawn_test_convergence_driver(
        &session_a,
        state_a.clone(),
        vec![GROUP_ID.to_string()],
    );
