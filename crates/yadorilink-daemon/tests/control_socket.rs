...
mod unix_socket_tests {
    ...
    use yadorilink_ipc_proto::daemonctl::{
        ...
        ListLinksRequest, ListTrashRequest, ListVersionsRequest, OverrideRequest, PauseRequest,
        PinRequest, RestoreTrashRequest, RestoreVersionRequest, ResumeRequest, RevertRequest,
        SetModeRequest, SetRetentionPolicyRequest, StatusRequest, UnlinkRequest, UnpinRequest,
    };
    ...
    #[tokio::test]
    async fn link_with_content_defined_chunking_round_trips_through_list() {
        let (socket_path, dir) = start_daemon().await;
        let folder = dir.path().join("vms");
        std::fs::create_dir_all(&folder).unwrap();
        ...
        assert_eq!(list.links.len(), 1);
        assert_eq!(list.links[0].chunking_policy, "content_defined");
    }

    /// A link created without `--content-defined-chunking` (the default) still
    /// reports `fixed` — the pre-existing behavior, unaffected by this field
    /// existing at all.
    #[tokio::test]
    async fn link_without_content_defined_chunking_defaults_to_fixed() {
        let (socket_path, dir) = start_daemon().await;
        let folder = dir.path().join("photos");
        std::fs::create_dir_all(&folder).unwrap();
        ...
        assert_eq!(list.links[0].chunking_policy, "fixed");
    }
    ...
    /// are seeded directly via `SyncState` (simulating what a gated
    /// incoming change / gated local edit would have recorded) rather than
    /// through a live second peer session — the actual gating that
    /// *produces* these entries is covered by `yadorilink-sync-core`'s own
    /// `peer_session` integration tests and this crate's `link_manager`
    /// unit tests; this test is specifically about the daemon control-IPC
    /// surface for reconciling them.
    #[tokio::test]
    async fn set_mode_then_override_and_revert_round_trip_through_status() {
        ...
        let folder = dir.path().join("publish");
        std::fs::create_dir_all(&folder).unwrap();
        let local_path = folder.to_string_lossy().to_string();
        send(
            &socket_path,
        ...
        .await;
        assert!(matches!(resp.payload, Some(RespPayload::SetMode(_))));

        let resp = send(&socket_path, ReqPayload::Status(StatusRequest {})).await;
        let Some(RespPayload::Status(status)) = resp.payload else {
            panic!("wrong response variant")
        };
        assert_eq!(status.links[0].mode, "send_only");
        assert_eq!(status.links[0].out_of_sync_count, 0);

        state.sync_state.record_out_of_sync("group-mode", "leaked.txt", 100).unwrap();

        let resp = send(&socket_path, ReqPayload::Status(StatusRequest {})).await;
        let Some(RespPayload::Status(status)) = resp.payload else {
            panic!("wrong response variant")
        };
        assert_eq!(status.links[0].out_of_sync_count, 1);

        let resp = send(
            &socket_path,
            ReqPayload::Override(OverrideRequest { local_path: local_path.clone() }),
        )
        .await;
        ...
        state.sync_state.upsert_file("group-mode", &record).unwrap();
        state.sync_state.record_receive_only_changed("group-mode", "local-edit.txt", 200).unwrap();

        let resp = send(&socket_path, ReqPayload::Status(StatusRequest {})).await;
        let Some(RespPayload::Status(status)) = resp.payload else {
            panic!("wrong response variant")
        };
        assert_eq!(status.links[0].mode, "receive_only");
        assert_eq!(status.links[0].receive_only_changed_count, 1);

        let resp = send(&socket_path, ReqPayload::Revert(RevertRequest { local_path })).await;
    ...
    async fn override_and_revert_reject_the_wrong_mode() {
        ...
        send(
            ...
            ReqPayload::Link(LinkRequest {
                local_path: local_path.clone(),
                group_id: "group-wrong-mode".into(),
                on_demand: false,
                max_local_size_bytes: None,
                content_defined_chunking: false,
                mode: String::new(),
    ...
    async fn unlink_removes_the_link() {
        ...
        let resp = send(&socket_path, ReqPayload::Unlink(UnlinkRequest { local_path })).await;
    ...
    async fn evict_via_control_socket_succeeds_for_an_indexed_hydrated_file() {
        ...
        .await;
        assert!(matches!(resp.payload, Some(RespPayload::Evict(_))));
    ...
    async fn restore_version_without_a_version_seq_defaults_to_most_recent_superseded() {
        ...
        state.block_store.put(b"second content").unwrap();
        let mut v2_version = yadorilink_sync_core::version_vector::VersionVector::new();
        v2_version.increment("device-a");
        v2_version.increment("device-a");
        state
            ...
            .upsert_file_with_origin(
                ...
                &yadorilink_sync_core::types::FileRecord {
                    ...
                    version: v2_version,
                    blocks: vec![yadorilink_sync_core::types::BlockInfo {
                        hash: hex::decode(state.block_store.put(b"second content").unwrap())
                            .unwrap(),
                        offset: 0,
                        size: 14,
                    }],
    ...
    async fn set_retention_policy_persists_and_is_reflected_via_list_links() {
        ...
        let resp = send(
            &socket_path,
            ReqPayload::SetRetentionPolicy(SetRetentionPolicyRequest {
                local_path: local_path.clone(),
                keep_versions: 3,
                keep_days: 7,
            }),
        ...
        .await;
        assert!(matches!(resp.payload, Some(RespPayload::SetRetentionPolicy(_))), "{resp:?}");

        let policy = state.sync_state.retention_policy_for_group("group-retention").unwrap();
        assert_eq!(
            policy,
        ...
        let link = links.iter().find(|l| l.group_id == "group-retention").unwrap();
        assert_eq!(link.retention_policy.max_versions, 3);
    ...
    #[tokio::test]
    async fn link_with_keep_versions_and_keep_days_persists_the_retention_policy() {
        let (socket_path, dir, state) = start_daemon_with_state().await;
        let folder = dir.path().join("archive");
        std::fs::create_dir_all(&folder).unwrap();
        let local_path = folder.to_string_lossy().to_string();
        send(
            ...
            ReqPayload::Link(LinkRequest {
                local_path: local_path.clone(),
                group_id: "group-link-retention".into(),
                on_demand: false,
                max_local_size_bytes: None,
                content_defined_chunking: false,
                mode: String::new(),
        ...
        let policy = state.sync_state.retention_policy_for_group("group-link-retention").unwrap();
        assert_eq!(
            policy,
            Some(yadorilink_sync_core::index::RetentionPolicy { max_versions: 3, max_age_days: 7 })
        );
        ...
        send(
            ...
            ReqPayload::Link(LinkRequest {
                local_path: folder2.to_string_lossy().to_string(),
                group_id: "group-link-default-retention".into(),
                on_demand: false,
                max_local_size_bytes: None,
                content_defined_chunking: false,
                mode: String::new(),
        ...
        .await;
        let policy =
            state.sync_state.retention_policy_for_group("group-link-default-retention").unwrap();
        assert_eq!(
            policy,
            Some(yadorilink_sync_core::index::RetentionPolicy {
                max_versions: 10,
                max_age_days: 30
            })
        );
