...
/// Per-device signing key for the seeding fixture — MUST be the
/// `dst_dag_migrate_b2::signing_key_for` convention, because `connect`
/// pins exactly those verifying keys via `PinnedAuthenticator`; a change
/// signed with any other key is rejected at wire admission and the peer
/// never learns the seeded root.
fn signing_key(dev: &Device) -> SigningKey {
    dst_dag_migrate_b2::signing_key_for(&dev.id)
}

/// Everything the hydrating side needs to mirror one seeded holder file:
/// the index row, plus the retained DAG change and content version that
/// authored it (the authoring-identity invariant makes bare index rows
/// unrepresentable in a DAG-backed group, so the fixture must author its
/// seeds the way production does).
struct SeededFile {
    record: FileRecord,
    change: Change,
    version: FileVersion,
...
fn seed_holder_file(dev: &Device, path: &str, content: &[u8]) -> Result<SeededFile, String> {
    ...
    let block_hash = hex::decode(&hash_hex).map_err(|e| e.to_string())?;
    let record = FileRecord {
        path: path.to_string(),
        size: content.len() as u64,
        mtime_unix_nanos: 1,
        blocks: vec![BlockInfo { hash: block_hash.clone(), offset: 0, size: content.len() as u32 }],
        deleted: false,
    };
    // Author the seed exactly like production would: the content version's
    // meta mirrors the row (same mtime/size/blocks), the change parents on
    // the device's current heads, and the row is written carrying that
    // change as its authoring identity in one transaction.
    let version = FileVersion::new(
        vec![VersionBlock { hash: BlockHash(block_hash), size: content.len() as u32 }],
        content.len() as u64,
        FileMeta {
            mtime_unix_nanos: 1,
            exec_bit: false,
    ...
    // hydrating side can admit exactly this one change without needing the
    ...
    Ok(SeededFile { record, change, version })
}
...
async fn connect(rng: &mut StdRng, a: &Device, b: &Device) -> Result<(), String> {
    ...
    dst_dag_migrate_b2::wire_dag_session(&session_a, a.state.clone(), &device_ids, &group_ids);
    dst_dag_migrate_b2::wire_dag_session(&session_b, b.state.clone(), &device_ids, &group_ids);
...
async fn run_scenario(seed: u64, fault_profile: HydrationFaultProfile) -> Result<(), String> {
    ...
    for path in PLACEHOLDER_PATHS {
        ...
        next_content_id += 1;
        let seeded = seed_holder_file(&device_a, path, &content)?;
        seed_placeholder(&device_b, &seeded)?;
    }
...
fn hydration_under_fault_chaos_scenario() {
    ...
    // Skipped entirely for a fast targeted run -- see
    // `dst_support::corpus::should_skip_replay`'s doc comment. Deliberately
    // NOT folded into `targeted` above: that flag also bounds
    // `max_attempts` for the fresh sweep below, a different concern this
    // must not perturb.
