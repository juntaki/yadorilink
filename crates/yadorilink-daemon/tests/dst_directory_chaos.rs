...
/// The change that authored `path`'s current row on `device`, read back
/// right after the op was applied. This is the causal evidence the oracle
...
fn record_write_at(
    ...
    oracle.record_write(path, device_idx, content_id, authoring_of(device, path));
...
fn record_delete_at(
    ...
    oracle.record_delete(path, device_idx, authoring_of(device, path));
    touched_paths.insert(path.to_string());
}
...
async fn run_scenario(seed: u64, ops_per_run: usize) -> Result<(), String> {
    ...
    for round in 0..ops_per_run {
        ...
        match kind_roll {
            ...
            35..=44 => {
                ...
                // `all_files` intentionally stops targeting race-(d)'s old
                // path after move-out-vs-edit, but the product may resolve
                // the edit as the live old-path winner rather than only as a
                // conflict copy. A later whole-directory rename then
                // legitimately tombstones that extra live descendant in its
                // `FilesChanged` cascade. Track such actual cascade outputs
                // separately from `siblings`: omitting the tombstone leaves
                // the oracle's older write looking causally live forever even
                // though this round starts only after full convergence and
                // authors a real DAG descendant that supersedes it (stable
            ...
            50..=59 => {
                ...
                // leaves the scenario's timeline byte-identical -- but the oracle's
                // causal evidence is read out of the live index, and a local change
                // that has not finished flushing yet has no authoring hash to read.
                // Reading it early yielded `None`, which fails loud as "never
                // superseded" on a perfectly converged run.
            ...
            80..=89 => {
                ...
                // superseded" on a perfectly converged run.
                tokio::time::sleep(RACE_SETTLE).await;
                record_delete_at(&mut oracle, &mut touched_paths, &old_path, x, device_idx_of(x));
                record_write_at(
                    &mut oracle,
                    &mut touched_paths,
                    &old_path,
            ...
            _ => {
                ...
                tokio::time::sleep(RACE_SETTLE).await;
                record_delete_at(&mut oracle, &mut touched_paths, &old_path, x, device_idx_of(x));
                record_delete_at(&mut oracle, &mut touched_paths, &old_path, y, device_idx_of(y));
                all_files.remove(&old_path);
    ...
    violations.extend(oracle.check_no_loss_recursive(&content_table, &devices, GROUP_ID));
