//! Broad randomized two-device DST fuzzer: unlike
//! `dst_peer_reconcile_race.rs`, which reproduces one specific,
//! hand-crafted historical race, this scenario drives many randomized
//! rounds of local writes/deletes on *both* real, simulated devices — solo
//! (uncontested) and racing (concurrent, undispatched-vs-incoming) — over
//! a small pool of shared paths, looking for *new*, not-yet-known
//! instances of the same bug class this suite targets: "a durably observed
//! local write is never silently discarded except by a causally-later
//! write/delete." Both devices run
//! the real watcher-boundary/debounce/`LocalChangeProcessor` pipeline with
//! `PendingLocalChangeFlush` wired (the guard is always on here — this
//! scenario is about finding new bugs against the
...
//! making a legitimate causally-superseded outcome indistinguishable from
//! real data loss.
...
///
/// Measured: this was the entire `[NoLoss]` population of this scenario.
/// On seed 3298840576 the trace shows the product authoring both racing
/// sides as genuine concurrent siblings off the same parent and producing a
/// conflict copy (`copies=1`) -- nothing was ever lost; the oracle was
/// handed the wrong evidence. Adding this flush takes this scenario from
/// 15/32 variations violating to green.
///
...
/// not re-toggling a known fix). Deliberately not `RUST_LOG` -- that
/// suppresses this class of race (see `dst_trace`'s own doc comment).
fn dst_trace_path(path: &str, msg: impl FnOnce() -> String) {
...
impl ChaosDevice {
    ...
    async fn capture_undiscovered_local_change(&self, group_id: &str, path: &Path) {
        ...
        // The decisive line. This fallback reads the path straight off disk,
        // so `changed=false` means what it found there was ALREADY what the
        // index holds -- i.e. the local write it exists to rescue was gone
        // before it ever looked. `changed=true` means it did rescue one.
        // Whichever of these prints for a path that later shows up in a
        // `[NoLoss]` violation is the answer to "guard not reached, or bytes
        // already gone".
        let rel = path.strip_prefix(&self.root).unwrap_or(path).to_string_lossy().to_string();
        let on_disk_len = std::fs::symlink_metadata(path).ok().map(|m| m.len());
        dst_trace_path(&rel, || {
            format!(
                "undiscovered-change fallback on {}: on_disk_len={:?} authored={}",
                self.device_id, on_disk_len, changed
            )
        });
        if !changed {
            return;
...
impl PendingLocalChangeFlush for ChaosDevice {
    fn flush_pending_local_change<'a>(
        ...
        Box::pin(async move {
            ...
            // Third fork: what did the accumulator actually say? `Some`
            // means the write was still pending and is about to be flushed;
            // `None` sends us to the disk-authoritative fallback. A lost
            // write must pass through one of these two lines.
            dst_trace_path(rel_path, || {
                format!(
                    "flush handle on {}: accumulator_reply={}",
                    self.device_id,
                    if found.is_some() { "Some(pending)" } else { "None" }
                )
            });
...
/// Round-settle budget. It must sit above a real recovery delay this
/// scenario reliably reproduces: a `BlockRequest` datagram lost at the
/// transport layer during conflict resolution recovers only via
/// `DEFAULT_HYDRATION_TIMEOUT` (30s) plus `reconcile_one_file`'s own
/// bounded retry (the simulated watcher cannot re-observe `materialize`'s
/// own writes, so this is not self-echo churn). Production has no "N seconds or
    ...
    for round in 0..ops_per_run {
        ...
        match kind_roll {
            ...
            _ => {
                ...
                // Record x's causal evidence HERE, after its write has been
                // delivered and the round has settled -- NOT up where
                // `x_content_id` is minted, which is before
                // `deliver_local_write(x, ..)` has put any bytes on disk.
                //
                // The flush inside `authoring_of` is necessary but NOT
                // sufficient at that earlier position, and this was measured
                // rather than reasoned: with `authoring_of` settling through
                // `flush_pending_local_change` but the call left up there,
                // this scenario still reported 15/32, 9/32, 17/32. No flush
                // can author a write that has not happened yet. (In
                // `dst_three_device_mesh_chaos` the call already sits after
                // its `deliver_local_write`, so the flush alone took that
                // scenario green -- same defect, different shape.)
                oracle.record_write(
                    path,
                    device_idx_of(x),
                    x_content_id,
...
fn is_targeted_corpus_replay_skip() -> bool {
    dst_support::corpus::should_skip_replay() || std::env::var("DST_VARIATIONS").is_ok()
}
