...
fn run_sweep_sync(
    ...
    // `files` row (and so only stops contributing its blocks to `live`) once
    // `SyncState::expire_superseded_and_trashed_versions` actually deletes
    // it, and that retention sweep is exactly what
...
mod tests {
    ...
    async fn eviction_without_remote_lease_never_reaches_physical_reclaim() {
        ...
        let store = Arc::new(BarrierBlockStore {
            inner: FsBlockStore::new(store_dir.path()).unwrap(),
