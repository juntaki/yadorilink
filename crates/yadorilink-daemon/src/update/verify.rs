...
///
/// TODO(publisher-pinning): `expected_identity` is matched as a
/// substring of the free-text `Authority:` line, which a confusable or
/// similarly-named signer could satisfy. The stronger fix is to extract
/// the Team ID (the parenthesized token at the end of that line, e.g.
/// `Developer ID Installer: Example Corp (TEAMID1234)`) and require an
/// *exact* match against a dedicated signed manifest field (e.g.
/// `artifact_publisher_team_id`), never a substring of the full
/// authority text. Deferred: this needs a manifest schema addition and
/// real Developer ID Installer test fixtures to validate the extraction
/// pattern, both out of scope for the current caller-empty-identity
/// hardening in `manager::verify_platform_signature`.
...
///
/// TODO(publisher-pinning): like `verify_macos_signature`, this matches
/// `expected_identity` as a substring of `SignerCertificate.Subject`.
/// The stronger fix is to pin `SignerCertificate.Thumbprint` (or a
/// public-key hash) via a dedicated signed manifest field (e.g.
/// `artifact_publisher_thumbprint`) and require an exact match, which a
/// similarly-named but differently-issued certificate cannot satisfy.
/// Deferred for the same reason as the macOS TODO above.
