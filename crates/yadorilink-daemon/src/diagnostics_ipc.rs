...
async fn run_bounded(
    ...
    // Production runs `work` on a `spawn_blocking` thread bounded by
    // `timeout`. The deterministic simulator has no blocking-thread pool and a
    // real OS thread's timing would bleed into the virtual clock, so run
    // `work` inline behind an already-ready future — the `timeout` wrapper and
    // the resulting `Result<Result<_, JoinError>, Elapsed>` outcome shape are
    // preserved so the match below is identical either way.
    #[cfg(not(madsim))]
    let outcome = tokio::time::timeout(timeout, tokio::task::spawn_blocking(work)).await;
    #[cfg(madsim)]
    let outcome =
        tokio::time::timeout(timeout, std::future::ready(Ok::<_, tokio::task::JoinError>(work())))
            .await;
