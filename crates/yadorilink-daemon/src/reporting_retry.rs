...
/// Phase 4 `ReportingRetryJob` -- a thin wrapper around
...
impl ReportingRetryJob {
    ...
    pub(crate) async fn run_once(&self, _trigger: MaintenanceTrigger) -> RetrySweepOutcome {
...
pub fn spawn_periodic(state: Arc<DaemonState>) {
    crate::supervise::spawn_logged("reporting-queue-retry", async move {
        ...
        loop {
            ...
            job.run_once(MaintenanceTrigger::Interval).await;
        }
    });
}
