...
fn head_lamport(state: &ReplicaCoordinator) -> u64 {
    ...
    state.sqlite().dag_get_change(&heads[0]).unwrap().unwrap().lamport
}

/// The live projection of `PATH` on `state`, if present and not tombstoned.
fn live_record(state: &ReplicaCoordinator) -> Option<FileRecord> {
    state.file_index_repository().get_file(GROUP, PATH).unwrap().filter(|r| !r.deleted)
}
...
fn converged(state: &ReplicaCoordinator, winner_hash: &[u8], loser_hash: &[u8]) -> bool {
...
fn dump(label: &str, state: &ReplicaCoordinator) {
    let heads = state.sqlite().dag_group_heads(GROUP).unwrap();
    eprintln!("--- {label}: {} head(s), records:", heads.len());
    for r in state.file_index_repository().list_files(GROUP).unwrap() {
        eprintln!(
...
async fn run_scenario(announce_a_first: bool) -> Result<(i64, i64, String, String), String> {
    ...
    while tokio::time::Instant::now() < deadline {
        ...
        for _ in 0..12 {
            // Since the Convergence Engine split, admitting a change over
            // the wire (`handle_change_batch`) only enqueues a
