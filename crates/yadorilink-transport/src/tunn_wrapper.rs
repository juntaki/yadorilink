...
/// The seam between the transport hub (which owns sockets and demultiplexing)
/// and the WireGuard protocol engine (which owns per-peer handshake and
/// session state). Keeping this narrow means the engine — currently
/// `boringtun` behind [`WgTunnel`] — can be swapped without the hub or
/// `peer_channel` reaching into protocol internals. The hub's own MAC1
/// verification and receiver-index demux use only `boringtun`'s stateless
/// helpers (`Tunn::parse_incoming_packet`, `RateLimiter`), never this engine.
pub trait WireGuardEngine {
    /// Encrypts one application message into zero or more raw datagrams.
    fn encrypt_message(&mut self, payload: &[u8]) -> Result<Vec<Vec<u8>>, crate::TransportError>;
    /// Feeds one received datagram through the engine.
    fn handle_incoming(&mut self, datagram: &[u8]) -> IncomingResult;
    /// Drives internal timers, optionally producing a datagram to send.
    fn tick(&mut self) -> Option<Vec<u8>>;
    /// Produces a datagram now (handshake initiation or keepalive) to probe a
    /// candidate path.
    fn probe(&mut self) -> Option<Vec<u8>>;
}

impl WireGuardEngine for WgTunnel {
    fn encrypt_message(&mut self, payload: &[u8]) -> Result<Vec<Vec<u8>>, crate::TransportError> {
        WgTunnel::encrypt_message(self, payload)
    }
    fn handle_incoming(&mut self, datagram: &[u8]) -> IncomingResult {
        WgTunnel::handle_incoming(self, datagram)
    }
    fn tick(&mut self) -> Option<Vec<u8>> {
        WgTunnel::tick(self)
    }
...
impl WgTunnel {
    ...
    pub fn handle_incoming(&mut self, datagram: &[u8]) -> IncomingResult {
        ...
        while let Some(input) = next_input.take() {
            ...
            match self.tunn.decapsulate(None, &input, &mut buf) {
                ...
                TunnResult::Err(e) => {
                    // Expected on the shared per-device socket: a handshake
                    // initiation is offered to every channel and only the one
                    // holding the right static key decapsulates it, so the
                    // others land here; replays and late packets do too. Not
                    // actionable, so debug rather than warn.
                    tracing::debug!(error = ?e, "wireguard decapsulate error");
                    break;
                }
    ...
    /// (e.g. direct-path upgrade attempts) without waiting
...
mod tests {
    ...
    /// foundation: a datagram that is not a valid WireGuard
